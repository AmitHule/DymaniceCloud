...
import com.vwits.skoda.skodademo.NameController;
...
public class NameControllerTest {
  ...
  public void testControllerShouldAcceptRequiredParamsAndReturnValidResponse(){
    ...
    when(modelService.saveOrUpdate(model)).thenReturn(model);

    Model modela=nameController.savedetails(model);
  ...
  public void testControllerShouldReturnEmptyApiObjectWhenErrorOccurred(){
    when(modelService.saveOrUpdate(model)).thenThrow(new RuntimeException());
